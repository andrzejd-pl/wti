...
def avg_movies_by_genre(dataframe, genre):
    avg = pandas.pivot_table(dataframe, values='rating', aggfunc=numpy.average, index=['genre_' + genre])
    avg = avg.drop(0)
...
def avg_movie_by_user_and_genre(dataframe, user_id, genre):
    return {
        'user_id': user_id,
        'genre': genre,
        'rating': avg_movies_by_genre(dataframe.loc[lambda df: df.user_id == user_id, :], genre)['rating']
    }
